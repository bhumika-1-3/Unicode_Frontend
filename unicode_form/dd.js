...
var pV=price.value;
var tV=title.value;
var qV=quantity.value;
  // to check if the user has completed input 
   if(income.value==0 || saving.value==0){
...
submitIS.addEventListener("click",function(){
 var iV=income.value;
var sV=saving.value;
  var incoming=Number(income.value);
var savings=Number(saving.value);
...
function reduceAmt(e){
  var incoming=Number(income.value);
  var savings=Number(saving.value); 
var amt=Number(price.value*quantity.value);
  if(income.value>=0 && amt<=income.value){
  console.log(income.value-amt);
  income.value=Number(income.value-amt);
  incomeDisplay.innerHTML=Number(income.value);
  balance.innerHTML=Number(incoming+savings);
}
  else if(saving.value>=0 && amt<=saving.value)
  {
    saving.value=Number(saving.value-amt);
    savingDisplay.innerHTML=Number(saving.value);
  } 
  else if(savingDisplay+incomeDisplay>=amt)
  {
    saving.value=Number(saving.value-amt+income.value);
    savingDisplay.innerHTML=saving.value;
    income.value="0";
